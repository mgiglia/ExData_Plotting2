...
tot.emissions.by.year.Baltimore <- dcast(main.melt.Baltimore, year ~ variable, fun.aggregate = sum)
tot.emissions.by.year.Baltimore

png(file = "plot2.png", width = 1400, height = 720, units = "px")
with(tot.emissions.by.year.Baltimore, 
     plot(year, Emissions
          ,xlab = "Year"
          ...
          ,xlim = c(1999,2008)
          ,xaxt = "n"))
axis(side = 1, at=tot.emissions.by.year.Baltimore$year, labels = tot.emissions.by.year.Baltimore$year)
with(tot.emissions.by.year.Baltimore, abline(lm(Emissions ~ year), lty=2, col="red"))
legend("topright"
       ,col = c("red")
       ,lty = 2
       ##,xjust = .5
       ,legend = c("Trend Line"))
title(main="Total Emissions by Year in Baltimore", sub="Emissions from All Sources, Where fips == '24510'")
dev.off()