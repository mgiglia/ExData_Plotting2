...
motor.vehc.balt.LA <- subset(main.melt, 
                             (SCC.Level.One == "Mobile Sources") 
                             & (SCC.Level.Two %in% c("Highway Vehicles - Diesel", "Highway Vehicles - Gasoline")) 
                             & (fips %in% c("24510","06037")))
motor.vehc.balt.LA <- mutate(motor.vehc.balt.LA, city = ifelse(motor.vehc.balt.LA$fips == "24510", "Baltimore City", "Los Angeles County"))

g <- ggplot(data=motor.vehc.balt.LA, aes(year, value)) + theme_bw() + theme(legend.position = "top")
g <- g + geom_jitter(aes(color=city), alpha = 0.75)
g <- g + geom_smooth(aes(color=city), linetype = 2, method="lm", size = 1.5)
g <- g + scale_x_continuous(breaks=motor.vehc.balt.LA$year) + theme(axis.text.x = element_text(color="dark gray"), axis.ticks.x = element_line(color="dark gray"))
g <- g + labs(title="Comparison of Motor Vehicle Emissions\n Balitmore City and Los Angeles County", color="City", y = "Natural Log of Emissions (PM25-PRI)", x="Year")
g <- g + scale_y_continuous(trans=log2_trans())

##print the plot
png(file = "plot6.png", width = 1400, height = 720, units = "px")
g
dev.off()